...
class BlogRepositoryImpl implements BlogRepository {
  Future<Either<String, Blog>> viewBlog(int id) async {
    Either<String, BlogModel> result =
        await BlogRemoteDataSourceImpl().viewBlog(id);
    var output;
    bool iserror = false;
  ...
  Future<Either<String, List<Blog>>> viewAllBlogs() async {
    Either<String, List<BlogModel>> result =
        await BlogRemoteDataSourceImpl().viewAllBlogs();
    List<BlogModel> dataList = [];
    List<Blog> output = [];
    bool iserror = false;

    print('extracting from repo....');
    result.fold((error) => iserror = true, (data) => dataList = data);
